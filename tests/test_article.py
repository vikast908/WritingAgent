...
def test_article_evidence_report_autogenerated(tmp_brain, fake_llm):
    """A finished article auto-writes evidence_report.md (the shareable trust artifact)."""
    cfg, settings = load_config(), load_settings()
    aid = orchestrator.start_article(cfg, settings, "u", "topic", _angle(),
                                     "ev", 1, 1, autonomous=True)
    orchestrator.run(cfg, "u", aid, log=_silent)
    p = ArticlePaths(aid, "u").root / "evidence_report.md"
    assert p.exists() and "Evidence report" in p.read_text(encoding="utf-8")
