...
Status: **proposal for discussion** (not adopted). Author: craft-engine design pass, 2026-06-16.
Context: written while building the register/genre layer (`registers.py`, `craft.py`) on
branch `feat/craft-engine-all-tiers`. This evaluates three ideas the owner raised and gives
an honest verdict on each, including where they're redundant or would backfire.
