...
| **Structured-output truncation recovery (2026-06-17; ceiling raised 2026-07-16)** | A reasoning model (e.g. `deepseek-v4-pro`) spends tokens *thinking* before it emits the JSON; if that fills `max_tokens` the reply is empty / cut off mid-object with `finish_reason=length`. `complete_structured` **and `complete_text`** now detect this and **raise `max_tokens`** (double, capped at the shared `llm._LEN_RETRY_CEILING` = 32000) then retry the **same** model/prompt - no repair turn (the prompt was fine) - so the call stays on its routed (stronger) tier instead of burning its retries on the same budget and degrading to the flash fallback. The ceiling was raised from a hard 16000 because the chapter writer *requests* 16000, so a `<16000` recovery could never fire for it (a truncated chapter re-sent the same doomed budget); 32000 gives real headroom, and a provider that rejects the larger budget 400s cheaply → fallback, still strictly better. Complemented by a `models.yaml` `max_tokens:` floor for the reasoning judgment nodes (`critic`/`judge`/`verifier` = 8000). Confirmed live: the first real OpenRouter run hit this exactly once (sec05) and the fix recovers on-tier in one extra try. |
...
| **Review-sweep invariants (2026-07-16)** | Durable decisions from a full-codebase redundancy/mismatch/optimization review. **Score-array alignment:** the per-unit `state["scores"]`/`["insights"]` arrays are the source of truth for the weakest-unit `revise` target and the summary card, so they MUST stay 1:1 with `committed`; any commit outside the attempt loop (escalation approval) appends via `common._record_escalated_score` (fed by the crit `_escalate` stashes). **Citation guard:** the humanizer/surgery preservation guard matches `[N?\d+]` (both plain `[12]` and synthesis `[N12]`), mirroring `polish._INLINE_CITE`. **Register-aware surgery:** the surgical show-don't-tell/de-passive guard uses the register's tell matcher, not the neutral one, so a word the register permits isn't rejected as new slop. **Panel parity:** `agentic_critique_panel` runs for books too (`critique_chapter` takes a `lens`); the fact-check panel stays article-only (a chapter has no per-unit research source to verify against). **Learned-policy hygiene:** a `revise` re-commit is marked `revised` in the trace and excluded from the first-pass reward so it can't teach "research hurts". **DB safety:** `Store.open` closes its connection if schema init raises (no leaked handle/lock on a corrupt/synced-folder db). **Single-source lexicon:** the critic prompt's example tells are generated from `slop.BANNED_VERBS/TRANSITIONS`; export deliverable filenames from `brain.EXPORT_DELIVERABLE_BY_FORMAT`. |
...
| **Portability / cost** | Zero *required* deps - the stdlib fetch path keeps CI green on all three OSes x Python 3.10-3.13. Scrapo is an optional extra (`pip install '.[deep]'` - on PyPI as `scrapo-ai`, import `scrapo`; Python 3.11+) for higher-fidelity fetching, alongside its Playwright browser tier. Deep mode adds one query-planning LLM call + N page fetches per unit - hence opt-in. In fake/offline mode the whole path no-ops. |
